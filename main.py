...
class WaitTaskMsg:
  ...
  async def build(self):
    self.message = await self.chan.send(f":Waiting_for_this_task_to_finish:  {self.text or 'Please wait...'}")
...
async def AddExperience(channel, uid, amount):
    ...
    if al > bl:
      ...
      try:
        embed = discord.Embed()
        embed.title = f":nom_party: Level {al}!"
...
async def SummonTrivia(channel=829389041623105616, questions=5, delay=60):
    ...
    if playingtrivia:
      await channel.send(f"{emojis.woot} Tried to start a game of trivia, but there is already one right now.")
      return
    if questions > 50:
    ...
    for i in range(qc):
      ...
      embed.description = "Answer: **`" + (question[1]) + "`**\n*" + str(len(correctanswers)) + " out of " + str(len(answers)) + " people answered correctly.*\n\n" + (":nom_party: **Get ready for the next question!**" if i != qc - 1 else ":thankyou: **Thank you for playing! Wait to see the results...**")
...
async def on_member_join(member):
    ...
    for inv in invitelist:
      ...
      if not invexists:
        ...
        try:
          await inviter.send("Thank you for inviting " + sanitize(member.name) + f" to Study Fam! You have been given {emojis.famcoin2} `300`.")
        except:
          pass
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.BadArgument):
      ...
      await ctx.send(f"{emojis.woot} You must be in the casino channel to do this!\nYou may have to unlock it in the shop.")
...
async def stats(ctx, user: discord.Member=None):
    ...
    embed.add_field(name=((f"{emojis.famcoin2} Coins" if user.id != 824316055681761320 else ":cat_popcorn: Popcorn bank") if user.id != 577934880634306560 else ":1150_pugdancel: Doggy bank") if user.id != 799293092209491998 else "🐖 Piggy bank", value=f"**{GetUserCoins(user.id):,d}**")
...
async def help(ctx, *, showcategory=None):
    ...
    if showcategory is None:
      ...
      if chosencateg is not None:
        ...
        embed.description = f"{emojis.woot} Uh, I could not find that category. Use `mom help` to see a list of categories."
...
async def StarMessage(message, nominator):
    ...
    try:
        await nominator.send(embed=discord.Embed(
            ...
            description=f"Do you want to pay {emojis.famcoin2} `{starcost}` coins to star [{sanitize(message.author.name)}'s message]({message.jump_url})?\nTheir message will display in {bot.get_channel(channels.BeaverBoard).mention}.\nType `yes` to confirm and place the message there.",
            colour=0x721806
            ))
    except:
    ...
    while True:
      ...
      try:
        ...
        if char in usedchars or char in usedcharsgood:
          await ctx.send(embed=discord.Embed(
            title="Letter in use",
            description=":goodbye_bush: You have already used this letter.",
            colour=discord.Colour.red()
          ))
          continue
        ...
        if not missingchars:
          ...
          await ctx.send(embed=discord.Embed(
            title=f"{emojis.wow} " + selectedword.upper(),
...
async def roleshop(ctx):
    ...
    for itemname in shopitems:
      ...
      embed.description += f"\n☞ [`{itemname}`](https://. \"Role name\") {emojis.famcoin2} `{item[0]:,d}`" + (f" • **Lasts {GetTimeString(item[3])}**" if len(item) >= 4 else "") + (f" (**[OWNED](https://. \"You own this role, and you cannot buy it\")**)" if discord.utils.get(ctx.author.roles, id=item[2]) else "") + f"\n{item[1]}\n"
...
async def buyrole(ctx, itemname=None):
    if itemname is None:
      await ctx.send(f"{emojis.woot} You have to actually include a role to buy it.")
      return
    itemname = itemname.lower()
    if not itemname in shopitems:
      await ctx.send(f"{emojis.woot} That role does not exist in the role shop, please view the shop to see what we have.")
      return
    item = shopitems[itemname]
    itemrole = discord.utils.get(bot.guilds[0].roles, id=item[2])
    if itemrole in ctx.author.roles:
      await ctx.send(f"{emojis.woot} You already have this! You cannot buy it.")
      return
    if GetUserCoins(ctx.author.id) < item[0]:
      await ctx.send(f"{emojis.woot} You don't have enough coins to buy this!")
      return
    await AddExperience(ctx, ctx.author.id, 300)
    ...
    await ctx.send(ctx.author.mention, embed=discord.Embed(
      title=f"{emojis.famcoin2} Purchased {itemname}",
...
async def packshop(ctx):
    ...
    for packname in tradecards.packs:
      ...
      embed.description += f'\n☞ [`{packname}`](https://. "Pack name") {emojis.famcoin2} `{pack["cost"]:,d}` ({len(pack["cards"])} possible cards)' + (f' - **{round((len(cardsownedc) / len(pack["cards"])) * 100)} % collected**' if len(cardsownedc) > 0 else "") + f'\n{pack["description"]}\n'
    ...
    while not chosencard:
      ...
      for card in possiblecards:
        ...
        if chance <= tradecards.rarity_chances[cardobj["rarity"]]:
          ...
          omsg = await ctx.send(f'Bought pack for `{pack["cost"]:,d}` coins.\n{emojis.doge_dance} **Opening the pack `{packname}` ...** {emojis.doge_dance}')
...
async def setgoal(ctx, *, ftime=None):
    if (GetUserAttr(ctx.author.id, "dailygoal") or [0, 0, 0])[2] == math.floor(time.time() / 24 / 60 / 60):
      await ctx.send(f"{emojis.woot} You already have a goal for today. Type `mom mygoal` for more information.")
      return
    if ftime is None:
      await ctx.send(f"{emojis.woot} You must provide a time amount to set todays goal to.\nExample: `2h 30m`")
      return
    totaltime = StringToTime(ftime)
    if totaltime < 5 * 60:
      await ctx.send(f"{emojis.woot} Your goal must be at least 5 minutes.")
      return
    if totaltime > 20 * 60 * 60:
      await ctx.send(f"{emojis.woot} What are you doing??\nDon't put your goal that high, max 20 hours.")
      return
    if (time.time() + totaltime + (30 * 60)) > int((time.time() / (24 * 60 * 60)) + 1) * 24 * 60 * 60:
      await ctx.send(f"{emojis.woot} Oops! I'm sorry, but if you would start studying now until you reach the goal, you would finish later than 23:30 in UTC+0 which is 30 minutes before goal reset. Therefore you cannot set that goal at the moment.")
      return
    ...
    await AddExperience(ctx, ctx.author.id, 200)
    await ctx.send(ctx.author.mention, embed=discord.Embed(
      description=f"{emojis.wow} Your goal of `{GetTimeString(totaltime)}` has been made, start studying and try[*](https://discord.com/channels/712808127539707927/713177565849845849/822971544098963456 \"Click for motivation\") reach it!\nSee your progress with the command `mom mygoal`.",
      colour=discord.Colour.green()
    ))
...
async def daily(ctx):
    ...
    if dailyclaim == todayint:
      await ctx.send(f"{emojis.woot} You have already claimed your daily reward!\nYou can claim it again in **{GetTimeString(((todayint + 1) * 60 * 60 * 24) - time.time())}**.")
      return
...
async def NewPrestige(uid, prestige):
    ...
    if prestige not in userprestiges:
      ...
      embed.description = f":nom_party: *{prestige[1]}*\n`+{prestige[2]} experience`"
...
async def makequestion(ctx):
    ...
    cmsg = await ctx.send(embed=discord.Embed(
      description=":shibahey: You're adding a question to trivia. Please read [the guidelines](https://discord.com/channels/712808127539707927/713177565849845849/813705379191455795 \"Show the guidelines\") before submitting your trivia question!\nType `confirm` if you have read the guidelines and want to continue making the trivia question.",
      colour=0x1c684d
    ))
    def check(m):
      return m.author.id == ctx.author.id and m.channel.id == ctx.channel.id
    try:
      ...
      if m == "confirm":
        eqmsg = await ctx.send(ctx.author.mention, embed=discord.Embed(
          description=f"{emojis.doge_dance} Type the __question__ for your trivia.\n*16-80 characters*",
        ...
        try:
          ...
          eamsg = await ctx.send(ctx.author.mention, embed=discord.Embed(
            description=f"{emojis.doge_dance} Type the __answer__ for your trivia.\n*1-18 characters*",
            colour=discord.Colour.purple()
          ))
          try:
            ...
            edmsg = await ctx.send(ctx.author.mention, embed=discord.Embed(
              description=f"{emojis.doge_dance} Set the __difficulty__ for your trivia.\n\n**1.** Easy\n**2.** Moderate\n**3.** Hard",
              colour=discord.Colour.purple()
            ))
            try:
              m = await bot.wait_for("message", timeout=60, check=check)
              ...
              embed = discord.Embed()
              embed.colour = discord.Colour.purple()
              embed.description = f"{emojis.doge_dance} Set the __genre__ for your trivia.\n"
              i = 1
...
async def finishbounty(ctx):
    ...
    provider = bot.guilds[0].get_member(doc.get("provider"))
    fsmsg = await ctx.send(f"{ctx.author.mention}\n:download1: Did `{sanitize(provider.name)}` complete their challenge?\nType `yes` or `no`.")
    def check(m):
      return m.author.id == ctx.author.id and m.channel.id == ctx.channel.id
    try:
      m = await bot.wait_for("message", timeout=30, check=check)
...
async def reminder(ctx, *, ftime=None):
    if timcol.count_documents({"owner": ctx.author.id}) >= 5:
      ...
      return
    await ctx.message.delete()
    tamsg = await ctx.send(ctx.author.mention + f"\n{emojis.doge_dance} How long should the timer be set to?\nE.g. `1h 30m`")
    def check(m):
      return m.author.id == ctx.author.id and m.channel.id == ctx.channel.id
    try:
      m = await bot.wait_for("message", timeout=60, check=check)
      ...
      if totaltime < 30:
        ...
        return
      tmmsg = await ctx.send(ctx.author.mention + f"\n{emojis.doge_dance} What should the message be?\nType `none` to ignore message.")
      try:
        m = await bot.wait_for("message", timeout=60, check=check)
      except asyncio.TimeoutError:
        await tmmsg.delete()
